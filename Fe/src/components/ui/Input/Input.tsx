...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input({ label, error, hint, leadingIcon, trailing, className = '', id, ...props }, ref) {
  ...
  return (
    <div className="w-full">
      {label && <label className="mb-2 block text-sm font-medium text-[#3f3f46]" htmlFor={inputId}>{label}{props.required && <span className="ml-1 text-danger" aria-hidden="true">*</span>}</label>}
      <div className="relative">
        {leadingIcon && <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[#879087]">{leadingIcon}</span>}
