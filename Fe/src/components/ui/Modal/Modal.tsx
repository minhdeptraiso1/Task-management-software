...
export function Modal({ open, title, description, children, onClose, showClose = true, actions }: ModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-black/55 p-4" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-labelledby="modal-title" className="animate-enter flex max-h-[92vh] w-full max-w-[600px] flex-col overflow-hidden rounded-xl bg-white shadow-2xl my-auto" onMouseDown={event => event.stopPropagation()}>
