...
export function Select({ label, options, className = '', id, ...props }: SelectProps) {
  ...
  return <div className="w-full">
    {label && <label className="mb-2 block text-sm font-medium text-[#3f3f46]" htmlFor={selectId}>{label}{props.required && <span className="ml-1 text-danger" aria-hidden="true">*</span>}</label>}
