...
export function subscribeRealtimeNotifications({ onNotification, onStatusChange }: SubscribeOptions) {
    ...
    try {
      ...
      socket.addEventListener('message', event => {
        ...
        for (const frame of frames) {
          ...
          try {
            const realtimeEvent = JSON.parse(body) as RealtimeNotificationEvent
            onNotification(realtimeEvent)
          } catch {}
        }
      })
